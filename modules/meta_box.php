...
if ( !class_exists( 'rsmSurveyDataMetaBox' ) ){
 ...
 if( !class_exists( 'rsmAssesseeDataMetaBox' ) ){
	class rsmAssesseeDataMetaBox{
			...
			foreach( $this->fields_parameters as $single_field){
				...
				switch( $single_field['type'] ){
					...
					$out .= '<div class="form-group">
						...
						<input type="text" class="form-control input-xlarge" name="udata-'.$single_field['name'].'" id="'.$single_field['name'].'" value="'. $current_array[$single_field['name']] .'">  
